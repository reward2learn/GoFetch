...
export async function GET(req: NextRequest) {
  try {
    ...
    const [userOrders, totalOrders] = await Promise.all([
      prisma.order.findMany({
        ...
        select: {
          ...
          buyerId: true,
          request: {
            select: { title: true, category: true, imageUrls: true, fromCity: true, fromCountry: true, toCity: true, toCountry: true, deliveryType: true, pickupLocation: true },
          },
        },
      }),
      prisma.order.count({
    ...
    const normalizedOrders = userOrders.map((order) => ({
      ...
      imageUrls: order.request?.imageUrls || null,
